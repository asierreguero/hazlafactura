...
function save(){localStorage.setItem('facturalista',JSON.stringify(snapshot()));$('#saveState').textContent='Guardado localmente'}
...
const today=new Date(),due=new Date(Date.now()+30*864e5);$('#invoiceDate').value=today.toISOString().slice(0,10);$('#dueDate').value=due.toISOString().slice(0,10);$('#invoiceNumber').value=`FL-${today.getFullYear()}-001`;
ids.forEach(id=>$('#'+id).addEventListener('input',update));$('#addItem').addEventListener('click',()=>{state.items.push({description:'',quantity:1,price:0});renderItems();update()});$('#printBtn').addEventListener('click',()=>window.print());$('#exportBtn').addEventListener('click',()=>download(`factura-${$('#invoiceNumber').value}.json`,JSON.stringify(snapshot(),null,2),'application/json'));$('#importFile').addEventListener('change',e=>{const file=e.target.files[0];if(!file)return;const reader=new FileReader();reader.onload=()=>{try{load(JSON.parse(reader.result))}catch{alert('El archivo no es una copia válida de FacturaLista.')}};reader.readAsText(file)});$('#clearBtn').addEventListener('click',()=>{if(confirm('¿Borrar todos los datos guardados en este navegador?')){localStorage.removeItem('facturalista');location.reload()}});
const stored=localStorage.getItem('facturalista');if(stored){try{load(JSON.parse(stored))}catch{renderItems();update()}}else{renderItems();update()}
